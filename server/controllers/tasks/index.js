    ...
    async (req, res, next) => {
        try {
            ...
            if (notificationType === "email" || notificationType === "both"){
                if (!userData.email) return res.status(400).json({ error: 'Please do add valid Email' });
            }
            if (notificationType === "sms" || notificationType === "both") {
                if (!userData.phone) return res.status(400).json({ error: 'Please do add valid Phone number' });
            }
            ...
            for (let i = 1; i <=4; i++) {
                reminders.push( new Date((+current) + ((mins* i / 4) * 60 * 1000)));
            }
            let taskdata = {
                reminders,
